...
public class CirSim extends Frame
  ...
  MouseMotionListener, MouseListener, ItemListener, KeyListener {
	...
	return "Calculadora de Circuitos - A3 UNISOCIESC";
    }
    ...
    String mouseModeStr = "Selecionar";
...
//    Vector setupList;
    ...
    CirSim(Circuit a) {
	super("SIMULADOR DE CIRCUITOS - A3 UNISOCIESC");
	applet = a;
	useFrame = false;
    }
	...
	try {
		...
		// Procurar circuito embutido na URL
		...
		if (in > 0) {
			...
			try {
				...
				System.out.println("não foi possível decodificar " + x);
				e.printStackTrace();
			}
		}
	    in = doc.lastIndexOf('/');
	...
	// m.add(exportLinkItem = getMenuItem("Exportar Link"));
	m.addSeparator();
	m.add(exitItem = getMenuItem("Sair"));

	m = new Menu("Editar");
	m.add(undoItem = getMenuItem("Desfazer"));
	undoItem.setShortcut(new MenuShortcut(KeyEvent.VK_Z));
	m.add(redoItem = getMenuItem("Refazer"));
	redoItem.setShortcut(new MenuShortcut(KeyEvent.VK_Z, true));
	m.addSeparator();
	m.add(cutItem = getMenuItem("Recortar"));
	cutItem.setShortcut(new MenuShortcut(KeyEvent.VK_X));
	m.add(copyItem = getMenuItem("Copiar"));
	copyItem.setShortcut(new MenuShortcut(KeyEvent.VK_C));
	m.add(pasteItem = getMenuItem("Colar"));
	pasteItem.setShortcut(new MenuShortcut(KeyEvent.VK_V));
	pasteItem.setEnabled(false);
	m.add(selectAllItem = getMenuItem("Selecionar Tudo"));
	...
	m = new Menu("Osciloscópio");
	if (useFrame)
		mb.add(m);
	else
		mainMenu.add(m);
	m.add(getMenuItem("Empilhar Todos", "stackAll"));
	m.add(getMenuItem("Desempilhar Todos", "unstackAll"));

	optionsMenu = m = new Menu("Opções");
	if (useFrame)
		mb.add(m);
		
	else
		mainMenu.add(m);
	m.add(dotsCheckItem = getCheckItem("Mostrar Corrente"));
	dotsCheckItem.setState(false); // hausen: alterado de true para false
	m.add(voltsCheckItem = getCheckItem("Mostrar Voltagem"));
	voltsCheckItem.setState(true);
	m.add(powerCheckItem = getCheckItem("Mostrar Potência"));
	m.add(showValuesCheckItem = getCheckItem("Mostrar Valores"));
	showValuesCheckItem.setState(true);
	//m.add(conductanceCheckItem = getCheckItem("Mostrar Condutância"));
	m.add(smallGridCheckItem = getCheckItem("Grade Pequena"));
	m.add(euroResistorCheckItem = getCheckItem("Resistores Europeus"));
	euroResistorCheckItem.setState(euro);
	m.add(printableCheckItem = getCheckItem("Fundo Branco"));
	printableCheckItem.setState(printable);
	m.add(conventionCheckItem = getCheckItem("Movimento Convencional da Corrente"));
	conventionCheckItem.setState(convention);
	m.add(idealWireCheckItem = getCheckItem("Fios Ideais"));
	...
	idealWireCheckItem.addItemListener(
		new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				System.err.println("fios ideais: " + WireElm.ideal);
			}
		}
	);


	m.add(optionsItem = getMenuItem("Outras Opções..."));
	...
	mainMenu.add(getClassCheckItem("Adicionar Fio", "WireElm"));
	mainMenu.add(getClassCheckItem("Adicionar Resistor", "ResistorElm"));

	Menu passMenu = new Menu("Componentes Passivos");
	mainMenu.add(passMenu);
	passMenu.add(getClassCheckItem("Adicionar Capacitor", "CapacitorElm"));
	passMenu.add(getClassCheckItem("Adicionar Indutor", "InductorElm"));
	passMenu.add(getClassCheckItem("Adicionar Interruptor", "SwitchElm"));
	passMenu.add(getClassCheckItem("Adicionar Botão de Pressão", "PushSwitchElm"));
	passMenu.add(getClassCheckItem("Adicionar Interruptor SPDT", "Switch2Elm"));
	passMenu.add(getClassCheckItem("Adicionar Potenciômetro", "PotElm"));
	passMenu.add(getClassCheckItem("Adicionar Transformador", "TransformerElm"));
	passMenu.add(getClassCheckItem("Adicionar Transformador com Tap",
					"TappedTransformerElm"));
	passMenu.add(getClassCheckItem("Adicionar Linha de Transmissão", "TransLineElm"));
	passMenu.add(getClassCheckItem("Adicionar Relé", "RelayElm"));
	passMenu.add(getClassCheckItem("Adicionar Memristor", "MemristorElm"));
	passMenu.add(getClassCheckItem("Adicionar Faísca", "SparkGapElm"));

	Menu inputMenu = new Menu("Entradas/Saídas");
	mainMenu.add(inputMenu);
	inputMenu.add(getClassCheckItem("Adicionar Terra", "GroundElm"));
	inputMenu.add(getClassCheckItem("Adicionar Fonte de Tensão (2 terminais)", "DCVoltageElm"));
	inputMenu.add(getClassCheckItem("Adicionar Fonte de A/C (2 terminais)", "ACVoltageElm"));
	inputMenu.add(getClassCheckItem("Adicionar Fonte de Tensão (1 terminal)", "RailElm"));
	inputMenu.add(getClassCheckItem("Adicionar Fonte de A/C (1 terminal)", "ACRailElm"));
	inputMenu.add(getClassCheckItem("Adicionar Onda Quadrada (1 terminal)", "SquareRailElm"));
	inputMenu.add(getClassCheckItem("Adicionar Saída Analógica", "OutputElm"));
	inputMenu.add(getClassCheckItem("Adicionar Entrada Lógica", "LogicInputElm"));
	inputMenu.add(getClassCheckItem("Adicionar Saída Lógica", "LogicOutputElm"));
	inputMenu.add(getClassCheckItem("Adicionar Relógio", "ClockElm"));
	inputMenu.add(getClassCheckItem("Adicionar Varredura de A/C", "SweepElm"));
	inputMenu.add(getClassCheckItem("Adicionar Tensão Variável", "VarRailElm"));
	inputMenu.add(getClassCheckItem("Adicionar Antena", "AntennaElm"));
	inputMenu.add(getClassCheckItem("Adicionar Fonte de Corrente", "CurrentElm"));
	inputMenu.add(getClassCheckItem("Adicionar LED", "LEDElm"));
	inputMenu.add(getClassCheckItem("Adicionar Lâmpada (beta)", "LampElm"));

	Menu activeMenu = new Menu("Componentes Ativos");
	mainMenu.add(activeMenu);
	activeMenu.add(getClassCheckItem("Adicionar Diodo", "DiodeElm"));
	activeMenu.add(getClassCheckItem("Adicionar Diodo Zener", "ZenerElm"));
	activeMenu.add(getClassCheckItem("Adicionar Transistor (bipolar, NPN)", "NTransistorElm"));
	activeMenu.add(getClassCheckItem("Adicionar Transistor (bipolar, PNP)", "PTransistorElm"));
	activeMenu.add(getClassCheckItem("Adicionar Amp Op (- no topo)", "OpAmpElm"));
	activeMenu.add(getClassCheckItem("Adicionar Amp Op (+ no topo)", "OpAmpSwapElm"));
	activeMenu.add(getClassCheckItem("Adicionar MOSFET (canal n)", "NMosfetElm"));
	activeMenu.add(getClassCheckItem("Adicionar MOSFET (canal p)", "PMosfetElm"));
	activeMenu.add(getClassCheckItem("Adicionar JFET (canal n)", "NJfetElm"));
	activeMenu.add(getClassCheckItem("Adicionar JFET (canal p)", "PJfetElm"));
	activeMenu.add(getClassCheckItem("Adicionar Chave Analógica (SPST)", "AnalogSwitchElm"));
	activeMenu.add(getClassCheckItem("Adicionar Chave Analógica (SPDT)", "AnalogSwitch2Elm"));
	activeMenu.add(getClassCheckItem("Adicionar SCR", "SCRElm"));
	//activeMenu.add(getClassCheckItem("Adicionar Varactor/Varicap", "VaractorElm"));
	activeMenu.add(getClassCheckItem("Adicionar Diodo Túnel", "TunnelDiodeElm"));
	activeMenu.add(getClassCheckItem("Adicionar Triodo", "TriodeElm"));
	//activeMenu.add(getClassCheckItem("Adicionar Diac", "DiacElm"));
	//activeMenu.add(getClassCheckItem("Adicionar Triac", "TriacElm"));
	//activeMenu.add(getClassCheckItem("Adicionar Fotoresistor", "PhotoResistorElm"));
	//activeMenu.add(getClassCheckItem("Adicionar Termistor", "ThermistorElm"));
	activeMenu.add(getClassCheckItem("Adicionar CCII+", "CC2Elm"));
	activeMenu.add(getClassCheckItem("Adicionar CCII-", "CC2NegElm"));

	Menu gateMenu = new Menu("Portas Lógicas");
	mainMenu.add(gateMenu);
	gateMenu.add(getClassCheckItem("Adicionar Inversor", "InverterElm"));
	gateMenu.add(getClassCheckItem("Adicionar Porta NAND", "NandGateElm"));
	gateMenu.add(getClassCheckItem("Adicionar Porta NOR", "NorGateElm"));
	gateMenu.add(getClassCheckItem("Adicionar Porta AND", "AndGateElm"));
	gateMenu.add(getClassCheckItem("Adicionar Porta OR", "OrGateElm"));
	gateMenu.add(getClassCheckItem("Adicionar Porta XOR", "XorGateElm"));

	Menu chipMenu = new Menu("Chips");
	mainMenu.add(chipMenu);
	chipMenu.add(getClassCheckItem("Adicionar Flip-Flop D", "DFlipFlopElm"));
	chipMenu.add(getClassCheckItem("Adicionar Flip-Flop JK", "JKFlipFlopElm"));
	chipMenu.add(getClassCheckItem("Adicionar LED de 7 Segmentos", "SevenSegElm"));
	chipMenu.add(getClassCheckItem("Adicionar VCO", "VCOElm"));
	chipMenu.add(getClassCheckItem("Adicionar Comparador de Fase", "PhaseCompElm"));
	chipMenu.add(getClassCheckItem("Adicionar Contador", "CounterElm"));
	chipMenu.add(getClassCheckItem("Adicionar Contador de Décadas", "DecadeElm"));
	chipMenu.add(getClassCheckItem("Adicionar Temporizador 555", "TimerElm"));
	chipMenu.add(getClassCheckItem("Adicionar DAC", "DACElm"));
	chipMenu.add(getClassCheckItem("Adicionar ADC", "ADCElm"));
	chipMenu.add(getClassCheckItem("Adicionar Latch", "LatchElm"));

	Menu otherMenu = new Menu("Outros");
	mainMenu.add(otherMenu);
	otherMenu.add(getClassCheckItem("Adicionar Texto", "TextElm"));
	otherMenu.add(getClassCheckItem("Adicionar Caixa", "BoxElm"));
	otherMenu.add(getClassCheckItem("Adicionar Sonda de Osciloscópio", "ProbeElm"));
	otherMenu.add(getCheckItem("Arrastar Todos (Alt-arrastar)", "DragAll"));
	otherMenu.add(getCheckItem(
			isMac ? "Arrastar Linha (Alt-S-arrastar, S-direita)" :
			"Arrastar Linha (S-direita)",
			"DragRow"));
	otherMenu.add(getCheckItem(
			isMac ? "Arrastar Coluna (Alt-\u2318-arrastar, \u2318-direita)" :
			"Arrastar Coluna (C-direita)",
			"DragColumn"));
	otherMenu.add(getCheckItem("Arrastar Selecionados", "DragSelected"));
	otherMenu.add(getCheckItem("Arrastar Posto (" + ctrlMetaKey + "-arrastar)",
				"DragPost"));


	mainMenu.add(getCheckItem("Selecionar/Arrastar Selecionados (espaço ou Shift-arrastar)", "Select"));
	main.add(mainMenu);

	main.add(resetButton = new Button("Resetar"));
	resetButton.addActionListener(this);
	dumpMatrixButton = new Button("Descarregar Matriz");
	//main.add(dumpMatrixButton);
	dumpMatrixButton.addActionListener(this);
	stoppedCheck = new Checkbox("Pausado");
	stoppedCheck.addItemListener(this);
	main.add(stoppedCheck);

	main.add(new Label("Velocidade da Simulação", Label.CENTER));

	// era máx de 140
	main.add(speedBar = new Scrollbar(Scrollbar.HORIZONTAL, 3, 1, 0, 260));
	speedBar.addAdjustmentListener(this);

	main.add(new Label("Velocidade da Corrente", Label.CENTER));
	currentBar = new Scrollbar(Scrollbar.HORIZONTAL,
							50, 1, 1, 100);
	currentBar.addAdjustmentListener(this);
	main.add(currentBar);

	main.add(powerLabel = new Label("Brilho", Label.CENTER));
...
//	setupList = new Vector();
	...
	elmMenu.add(elmScopeMenuItem = getMenuItem("Ver no escopo"));
	elmMenu.add(elmCutMenuItem = getMenuItem("Recortar"));
	elmMenu.add(elmCopyMenuItem = getMenuItem("Copiar"));
	elmMenu.add(elmDeleteMenuItem = getMenuItem("Deletar"));
	main.add(elmMenu);
	...
	m.add(getMenuItem("Romover", "remove"));
	m.add(getMenuItem("Velocidade 2x", "speed2"));
	m.add(getMenuItem("Velocidade 0.5x", "speed1/2"));
	m.add(getMenuItem("Escala 2x", "scale"));
	m.add(getMenuItem("Escala Máxima", "maxscale"));
	m.add(getMenuItem("Empilhar", "stack"));
	m.add(getMenuItem("Desempilhar", "unstack"));
	m.add(getMenuItem("Resetar", "reset"));
	if (t) {
	...
	if ( elm.needsShortcut() && s == 0 )
	{
	    if ( s == 0 )
	    {
		System.err.println("Sem atalho " + c + " para " + c);
		return;
	    }
	    else if ( s <= ' ' || s >= 127 )
	    {
		System.err.println("Atalho inválido " + c + " for " + c);
		return;
	    }
	}
	...
	if ( dumpTypes[t] != null && dumpTypes[t] != dclass ) {
	    System.out.println("Conflito de dump: " + c + " " +
			       dumpTypes[t]);
	    return;
	}
	dumpTypes[t] = dclass;
	...
	if ( elm.needsShortcut() && shortcuts[s] != null &&
	     shortcuts[s] != sclass )
	{
	    System.err.println("conflito de atalho: " + c +
	 		       " (Atribuído previamente para " +
			       shortcuts[s] + ")");
	}
	else
	{
	...
	*/
	...
	while (changed) {
	    ...
	    for (i = 0; i != elmList.size(); i++) {
		CircuitElm ce = getElm(i);
		// looop pelos nós para ver se eles estão conectados
		// em outros nós sem curto
	    ...
	    // conectar nós desconectadors
	    for (i = 0; i != nodeList.size(); i++)
		if (!closure[i] && !getCircuitNode(i).internal) {
		    System.out.println("nó " + i + " desconectado");
	...
	for (i = 0; i != elmList.size(); i++) {
	    CircuitElm ce = getElm(i);
	    // procurar indutores sem caminho para corrente
		...
		if (!fpi.findPath(ce.getNode(0), 5) &&
		    ...
		    System.out.println(ce + " sem caminho");
		    ce.reset();
		}
	    }
	    // procurar por fontes de corrente sem caminho para corrente
	    if (ce instanceof CurrentElm) {
		FindPathInfo fpi = new FindPathInfo(FindPathInfo.INDUCT, ce,
						    ce.getNode(1));
		if (!fpi.findPath(ce.getNode(0))) {
		    stop("Sem saída para fonte de corrente!", ce);
		    return;
		}
	    }
		...
		FindPathInfo fpi = new FindPathInfo(FindPathInfo.VOLTAGE, ce,
						    ce.getNode(1));
		if (fpi.findPath(ce.getNode(0))) {
		    stop("Fonte de tensão/loop de fio sem resistência!", ce);
		    return;
		}
	    }
		...
		FindPathInfo fpi = new FindPathInfo(FindPathInfo.SHORT, ce,
						    ce.getNode(1));
		if (fpi.findPath(ce.getNode(0))) {
		    System.out.println(ce + " em curto");
		    ce.reset();
		} else {
		    fpi = new FindPathInfo(FindPathInfo.CAP_V, ce, ce.getNode(1));
		    if (fpi.findPath(ce.getNode(0))) {
			stop("Loop de capacitor sem resistência!", ce);
			return;
		    }
		}
	    }
	...
	for (i = 0; i != matrixSize; i++) {
		...
		if (qm == -1) {
			...
			if (elt.type != RowInfo.ROW_NORMAL) {
			    ...
			    // about
			    System.out.println("swap falhou");
			    continue;
			}
		    }
	...
	if (Double.isNaN(r0) || Double.isInfinite(r0)) {
	    System.out.print("resistência ruim " + r + " " + r0 + "\n");
	    int a = 0;
	    a /= a;
	}
	...
	for (iter = 1; ; iter++) {
	    ...
	    if (subiter > 5)
		System.out.print("convirger depois de " + subiter + " iterações\n");
	    if (subiter == subiterCount) {
		stop("Convergência falhou!", null);
		break;
	    }
	    t += timeStep;
	    for (i = 0; i != scopeCount; i++)
	...
	if (e.getSource() == redoItem)
	    doRedo();
	if (ac.compareTo("Recortar") == 0) {
	    if (e.getSource() != elmCutMenuItem)
		menuElm = null;
	    doCut();
	}
	if (ac.compareTo("Copiar") == 0) {
	    if (e.getSource() != elmCopyMenuItem)
		menuElm = null;
	    doCopy();
	}
	if (ac.compareTo("Colar") == 0)
	    doPaste();
