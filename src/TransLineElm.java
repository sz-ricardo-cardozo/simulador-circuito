...
class TransLineElm extends CircuitElm {
	...
	if (voltageL == null) {
	    sim.stop("Atraso na linha de transmissão muito grande!", this);
	    return;
	}
	...
	if (Math.abs(volts[0]) > 1e-5 || Math.abs(volts[1]) > 1e-5) {
	    sim.stop("Linha de transmissão precisa de aterramento!", this);
	    return;
	}
    }
	...
	if (n == 0)
	    return new EditInfo("Atraso (s)", delay, 0, 0);
	if (n == 1)
	    return new EditInfo("Impedância (ohms)", imped, 0, 0);
	return null;
    }
